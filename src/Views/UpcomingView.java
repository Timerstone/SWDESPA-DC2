...
public class UpcomingView implements Initializable{
    ...
    public void refresh()
    {
    	...
    	for(int i = 0; i < model.getActivities().size(); i++)
    	{
    		...
    		if(!temp.getStatus() && temp.getDate().isAfter(LocalDate.now()) || (temp.getDate().equals(LocalDate.now()) && temp.getTimeEnd().isAfter(LocalTime.now())))
    		{
